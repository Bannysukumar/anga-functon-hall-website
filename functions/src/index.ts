...
function buildSyntheticInvoiceForCheckout(bookingData: Record<string, any>): Record<string, any> {
  ...
  const paid =
    bookingData.breakdown?.paidAmount ??
...
async function sendCheckoutEmail(
  ...
  const templateValues = {
    userName: String(
      invoiceForTemplate.customer?.name || bookingData.customerName || "Guest"
    ),
    bookingId: String(bookingData.id || ""),
    invoiceNumber: String(
      bookingData.invoiceNumber || invoiceForTemplate.invoiceNumber || ""
    ),
    ...
    paidAmount: Number(
      invoiceForTemplate.breakdown?.paidAmount ?? bookingData.advancePaid ?? 0
    ).toFixed(2),
...
async function sendBookingLifecycleEmail(params: {
  ...
  const bodyByEvent: Record<BookingLifecycleEvent, string> = {
    ...
    BOOKING_CONFIRMED:
      "<p>Hello {customerName},</p><p>Your booking is now confirmed.</p><p><strong>Booking ID:</strong> {bookingId}</p><p><strong>Venue:</strong> {listingTitle}</p><p><strong>Room:</strong> {roomNumber}</p><p><strong>Type:</strong> {roomType}</p><p><strong>Floor:</strong> {floor}</p><p><strong>Check-in:</strong> {checkInDate}</p><p><strong>Check-out:</strong> {checkOutDate}</p><p><strong>Total:</strong> INR {totalAmount}</p><p>Thank you for choosing Anga Function Hall.</p>",
    BOOKING_CANCELLED:
    ...
    BOOKING_CHECKOUT:
      "<p>Hello {customerName},</p><p>Thank you for staying at Anga Function Hall.</p><p>We hope you had a great experience.</p><p><strong>Booking ID:</strong> {bookingId}</p><p><strong>Room:</strong> {roomNumber}</p><p><strong>Checkout At:</strong> {checkOutAt}</p><p>Regards,<br/>Anga Function Hall</p>",
  }
  const roomNums = Array.isArray(booking.selectedRoomNumbers)
    ? (booking.selectedRoomNumbers as unknown[]).map((x) => String(x).trim()).filter(Boolean)
    : []
...
async function releaseBookingAvailabilityForCheckout(bookingId: string) {
...
async function finalizeCheckout(params: {
  ...
  let invoiceForEmail: Record<string, any> | null = null
  if (bookingData.invoiceId) {
    const invoiceSnap = await db.collection("invoices").doc(String(bookingData.invoiceId)).get()
    if (invoiceSnap.exists) {
      invoiceForEmail = { id: invoiceSnap.id, ...(invoiceSnap.data() || {}) } as Record<string, any>
    }
  }
  try {
    ...
    await bookingRef.set(
      {
        checkoutEmailStatus: "failed" as const,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
      { merge: true }
    )
 ...
 */
function scheduledFieldToMs(value: any): number | null {
...
async function finalizeAutoCheckIn(bookingId: string) {
  const ref = db.collection("bookings").doc(bookingId)
  await db.runTransaction(async (transaction) => {
    const snap = await transaction.get(ref)
    if (!snap.exists) return
    const data = snap.data() || {}
    if (String(data.status) !== "confirmed") return
    if (data.checkInAt != null) return
    const scheduledInMs =
      scheduledFieldToMs(data.scheduledCheckInAt) ?? scheduledFieldToMs(data.checkInDate)
    const scheduledOutMs =
      scheduledFieldToMs(data.scheduledCheckOutAt) ?? scheduledFieldToMs(data.checkOutDate)
    if (!scheduledInMs || !scheduledOutMs) return
    const now = Date.now()
    if (scheduledInMs > now) return
    if (scheduledOutMs <= now) return
    ...
    transaction.set(
      ref,
      {
        ...
        checkInAt:
          typeof data?.scheduledCheckInAt?.toDate === "function"
...
async function runScheduledAutoCheckIns() {
  ...
  const due = snap.docs.filter((docSnap) => {
    ...
    if (data.checkInAt != null) return false
    const scheduledInMs =
      scheduledFieldToMs(data.scheduledCheckInAt) ?? scheduledFieldToMs(data.checkInDate)
    const scheduledOutMs =
      scheduledFieldToMs(data.scheduledCheckOutAt) ?? scheduledFieldToMs(data.checkOutDate)
    if (!scheduledInMs || !scheduledOutMs) return false
    if (scheduledInMs > now.getTime()) return false
    if (scheduledOutMs <= now.getTime()) return false
    return true
  })
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (freshSnap.exists) {
            const fresh = { id: freshSnap.id, ...(freshSnap.data() || {}) } as Record<string, any>
            if (fresh.checkoutEmailStatus !== "sent") {
              let invoiceForEmail: Record<string, any> | null = null
              if (fresh.invoiceId) {
                const invSnap = await db.collection("invoices").doc(String(fresh.invoiceId)).get()
                if (invSnap.exists) {
                  invoiceForEmail = { id: invSnap.id, ...(invSnap.data() || {}) } as Record<string, any>
                }
              }
...
export const scheduledAutoCheckoutJob = onSchedule("every 1 minutes", async () => {
